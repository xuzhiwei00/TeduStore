...
<script type="text/javascript">
	...
	function goUpdate(id){
		$.ajax({
			...
			success:function(obj){
				$("#id").val(obj.date.id);
				$("#receiverName").val(obj.date.recvName);
				$("#receiverAddress").val(obj.date.recvAddress);
				$("#receiverMobile").val(obj.date.recvPhone);
				$("#receiverPhone").val(obj.date.recvTel);
				$("#receiverZip").val(obj.date.recvZip);
				$("#addressName").val(obj.date.recvTag);
				getProvinces(obj.date.recvProvince,obj.date.recvCity,obj.date.recvArea);
		...
		$.ajax({
			...
			success:function(obj){
				...
				for(i=0;i<obj.date.length;i++){
					if(obj.date[i].isDefault==1){																
						var str1 ='<div class="aim_content_one aim_active">'+
	                    '<span class="dzmc dzmc_active">'+obj.date[i].recvTag+'</span>'+
	                    '<span class="dzxm dzxm_normal">'+obj.date[i].recvName+'</span>'+
	                    '<span class="dzxq dzxq_normal">'+obj.date[i].recvDistrict+obj.date[i].recvAddress+'</span>'+
	                    '<span class="lxdh lxdh_normal">'+obj.date[i].recvPhone+'</span>'+
	                    '<span class="operation operation_normal">'+
	                    '<span class="aco_change" onclick="goUpdate('+obj.date[i].id+')">修改</span>|<span class="aco_delete" onclick="deleteAddress('+obj.date[i].id+')">删除</span>'+
	                    '</span>'+
	                    '<span class="swmr  swmr_normal" id='+obj.date[i].id+'></span>'+
	                    '</div>';
	                    $(".address_information_manage").append(str1);
					}else{
						var str2 = '<div class="aim_content_two">'+
	                    '<span class="dzmc dzmc_normal">'+obj.date[i].recvTag+'</span>'+
	                    '<span class="dzxm dzxm_normal">'+obj.date[i].recvName+'</span>'+
	                    '<span class="dzxq dzxq_normal">'+obj.date[i].recvAddress+'</span>'+
	                    '<span class="lxdh lxdh_normal">'+obj.date[i].recvPhone+'</span>'+
	                    '<span class="operation operation_normal">'+
	                    	'<span class="aco_change" onclick="goUpdate('+obj.date[i].id+')">修改</span>|<span class="aco_delete" onclick="deleteAddress('+obj.date[i].id+')">删除</span>'+
	                    '</span>'+
	                    '<span class="swmr  swmr_normal" id='+obj.date[i].id+'>设为默认</span>'+
	                    '</div>';
						$(".address_information_manage").append(str2);
					}
					$(".swmr_normal").click(function(){
						$.ajax({
							url:"${pageContext.request.contextPath}/address/getDefault.do",
							type:"get",
			...
			$.ajax({
				...
				success:function(obj){
					...
					for(var i= 0;i<obj.date.length;i++){
						var option = $("<option value="+obj.date[i].provinceCode+">"+obj.date[i].provinceName+"</option>");
	...
	function getCities(provinceCode,cityCode,areaCode){
		$.ajax({
			...
			success:function(obj){
                ...
                for(var i= 0;i<obj.date.length;i++){
			     var option = $("<option value="+obj.date[i].cityCode+">"+obj.date[i].cityName+"</option>");
	...
	function getAreas(cityCode,areaCode){
		$.ajax({
			...
			success:function(obj){
				...
				for(var i=0;i<obj.date.length;i++){
					$("#receiverDistrict").append("<option value="+obj.date[i].areaCode+">"+obj.date[i].areaName+"</option>")
				}
				if(areaCode!=-1){
